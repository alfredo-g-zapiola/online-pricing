...
# group number array : [ 1 ng1
  ...
  #simulazione active graph influenza con n step di influenza
    ...
    init_prob_matrix = np.random.uniform(0, 0.1, (number_clients.shape[0],number_clients .shape[0]))
    matrix_unbalanced=probability_matrix_unbalanced(init_prob_matrix,newly_active_customers, group_number)
 ...
 # serve dictionary che  contenga i 3 graph
  ...
  def probability_matrix_unbalanced(self, init_prob_matrix,initdata, group_number, number_clients):
    ...
    initdata[:][1]=initdata[:][1]-(group_number==2)*ng1*np.array(initdata.shape[0])-(group_number==3)*(ng1+ng2)*np.array(initdata.shape)
  ...
  def simulate_influence_history(self, n_steps_max, direct_clients, group_number, initdata):
    ...
    init_prob_matrix = np.random.uniform(0, 0.1, (number_clients.shape[0],number_clients .shape[0]))
    prob_matrix = probability_matrix_unbalanced(self, init_prob_matrix,initdata, group_number, number_clients)
